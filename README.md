...
- 双引擎朗读：「调用朗读」= 云端神经音色（晓涵/晓晓/云希等，需网络），逐句请求合成、下载完成后即播（含缓冲提示）；「本地朗读」= 系统语音（无需网络），点播放器按钮随时切换，云端不可用时自动降级
- 语音包朗读：把 24 篇资料按 ~150 字/段批量预生成音频（晓涵音色，32kbps 单声道），上传到阿里云 OSS 后按段离线播放；点击正文自动定位到对应音频段，进度条按“段”显示。生成与上传脚本见 `_work/gen_audio.py`、`_work/upload_oss.py`
