...
- 网页朗读：底部毛玻璃播放器（苹果音乐风格）「▶」大圆钮播放/暂停，支持上一句/下一句、0.75~2 倍速、**逐字高亮跟读**、动态均衡器动画
- 双模式朗读：「语音包」= 预生成音频（晓涵音色，离线按段播放）；「本地朗读」= 系统语音；两个模式按钮按下反色显示当前选中，点击正文任意字即可从对应位置开始读；播放器支持倍速候选菜单、音量滑条，页面左侧有阅读进度标尺（颜色由浅到深）
- 语音包朗读：把 24 篇资料按 ~150 字/段批量预生成音频，上传到阿里云 OSS 后按段离线播放；点击正文自动定位到对应音频段，进度条按“段”显示，10 秒无操作播放器自动收起。生成与上传脚本见 `_work/gen_audio.py`、`_work/upload_oss.py`
- 已读标记：朗读过的文字会变成淡绿色半透明底纹，与未读部分区分，并按篇记住朗读到第几句
- 目录与导航：播放器面板「☰ 目录」按钮，弹出毛玻璃底部抽屉，含「本文目录」（点标题跳转）与「全系列导航」（24 篇快速切换）两个 Tab
...
> 关于苹果朗读：网页“▶”按钮在 iPhone 上用「本地朗读」调用的就是 iOS 自带中文语音（与系统朗读同引擎）。
